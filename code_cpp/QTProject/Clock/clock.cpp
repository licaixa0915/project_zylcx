...
void Clock::paintEvent(QPaintEvent *event)
{
    ...
    int side = qMin(height(), width()); //绘制的范围(宽、高中最小值)
    ...
    painter.scale(side / 250.0, side / 250.0);//设定画布的边界,用窗体宽高的最小值来计算时钟的大小，防止窗体拉伸导致的时钟变形以及显示不全
    painter.setPen(Qt::red);   //填充时针，不需要边线所以NoPen
    QString timeStr= QTime::currentTime().toString();     //绘制当前的时间
    painter.drawText(-40,30,100,30,Qt::AlignHCenter | Qt::AlignTop, timeStr);
