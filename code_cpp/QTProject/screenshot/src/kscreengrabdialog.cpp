	...
	m_widgetTextEditMask(nullptr)
{
    ...
    m_toolBarHeaderManager = new MToolBarHeaderManager();
    m_toolBarHeaderManager->init(this, drawShape);
...
{
    if (m_toolBarHeaderManager)
        delete m_toolBarHeaderManager;
}
...
{
    connect(m_toolBarHeaderManager, &MToolBarHeaderManager::drawShapeChanged, this, &KScreenGrabDialog::onDrawShapeChanged);
    connect(m_toolBarHeaderManager, &MToolBarHeaderManager::sigToggleDrawAction, this, &KScreenGrabDialog::onToggleDrawAction);
}

void KScreenGrabDialog::onDrawShapeChanged(DrawShape shape)
{
    m_drawShape = shape;
}
...
{
    m_toolBarHeaderManager->toolBarHeader()->hideTooltip();
    if (m_toolBarHeaderManager->toolBarHeader()->isVisible() && m_toolBarHeaderManager->toolBarHeader()->geometry().contains(event->pos()))
	{
		m_badSelection = true;
		return;
	}
	...
	if (event->buttons() & Qt::LeftButton)
	{
		...
		else // m_selectmode != smSelection
		{
			if (m_drawShape == dsPolygon)
			{
                m_toolBarHeaderManager->hideToolBarHeader();
                m_toolBarHeaderManager->toolBarHeader()->hideTooltip();
...
{
    m_disableZoomBox = (m_toolBarHeaderManager->toolBarHeader()->isVisible() && m_toolBarHeaderManager->toolBarHeader()->geometry().contains(event->pos()));
...
void KScreenGrabDialog::reset()
{
	...
	m_polygonState = dpsEnd;
    m_toolBarHeaderManager->showToolBarHeader();
    m_toolBarHeaderManager->toggleDrawAction();
...
void KScreenGrabDialog::onToggleDrawAction()
{
	if (m_drawShape != dsRect)
	{
		m_rcSelection = QRect();
	...
	update();
    m_toolBarHeaderManager->showToolTip();
}
